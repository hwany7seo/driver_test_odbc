...
int main() {
    ...
    req = cci_prepare(conn, "DROP TABLE IF EXISTS test_table", 0, &err_buf);
    checkError(req, "Preparing Drop Statement");
    error = cci_execute(req, 0, 0, &err_buf);
    checkError(error, "Executing Drop Statement");
    ...
    req = cci_prepare(conn, "INSERT INTO test_table (id, name) VALUES (?, ?)", 0, &err_buf);
    checkError(req, "Preparing Insert Statement");

    for (id = 1; id <= TEST_COUNT; id++) {
        snprintf(name, sizeof(name), "oripdo%d", id);
        error = cci_bind_param(req, 1, CCI_A_TYPE_INT, &id, CCI_U_TYPE_INT, CCI_BIND_PTR);
        checkError(error, "Binding ID Parameter");

        error = cci_bind_param(req, 2, CCI_A_TYPE_STR, name, CCI_U_TYPE_STRING, CCI_BIND_PTR);
        checkError(error, "Binding Name Parameter");
